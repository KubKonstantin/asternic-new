...
?>
<?php
$event_query = "select callid, event, data1 from queue_log where time >= '$start' and time <= '$end' and event in ('COMPLETEAGENT', 'COMPLETECALLER', 'DID', 'ABANDON') order by callid DESC;";

$event_res = $connection->query($event_query);

$did_query = "select distinct(data1) from queue_log where event = 'DID';";
